...
/**
 * This is a simple class to take a gov.notify key or other arbitrary string representing a number
 * and a radix and return the number of bits of data in the key. This is primarily for working out
 * the validity of notify keys.
 *
 * @author innesm
 */
@ToString
public class BitCalculator {
  ...
  /**
   * A bag of stuff holding details of the supplied data. Could probably boil this down to just bits
   * and valid.
   */
  public static class KeyInfo {
     ...
     *
     * @param anInputString String to analyse
     * @param anValue Value represented by input string
     * @param anValueStr Value portion of input string
     * @param anKeyName Name of a key
     * @param anBits Number of bits represented by data in input string
     * @param anRadix Radix of number in input string
     * @param anValid Validity flag
     */
    public KeyInfo(
        final String anInputString,
        final BigInteger anValue,
        final String anValueStr,
        final String anKeyName,
        final int anBits,
        final int anRadix,
        final boolean anValid) {
      this.inputString = anInputString;
      this.value = anValue;
      this.valueStr = anValueStr;
      this.keyName = anKeyName;
      this.bits = anBits;
      this.radix = anRadix;
      this.valid = anValid;
    }
  ...
  /**
   * Parses a String into a KeyInfo structure. If the isKey flag is set the String is a gov.notify
   * key and the text up to the first hyphen is assumed to be the name of the key. If the isKey flag
   * is not set, the string is a number in base radix with optional hyphens to break it up.
   *
   * @param str string to parse
   * @param isKey key flag to indicate whether the string is a gov.notify key
   * @param radix the radix of the number portion
   * @return a KeyInfo object showing the number of bits of data contained within the number portion
  ...
  public KeyInfo analyse(final String str, final boolean isKey, final int radix) {
    ...
    if (info.valid) {
      ...
      return new KeyInfo(
          info.inputString, info.value, info.valueStr, info.keyName, bits, info.radix, valid);
    } else {
      return info;
    }
  }
  ...
  /**
   * Analyse a potential UUID. If it's a valid hex number and there are exactly 128 bits, the
   * returned structure will be have valid = true (otherwise false)
   *
   * @param hexString A number encoded as a hex string
   * @return an object showing the number of bits of data in the supplied string and a flag showing
  ...
  public KeyInfo analyseUUID(final String hexString) {
    ...
    if (info.bits != UUID_BITS && info.valid == true) {
      return new KeyInfo(
          info.inputString, info.value, info.valueStr, info.keyName, info.bits, info.radix, false);
    } else {
      return info;
    }
  }
  ...
  /**
   * Analyse a gov.notify key - a gov.notify key is a hex number with an alphanumeric prefix
  ...
  /**
   * A helper method to parse and tokenise a supplied string. If it's a key it'll strip of
   * everything to the first hyphen and use it as keyName otherwise it will parse the whole number
   *
