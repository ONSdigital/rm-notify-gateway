...
import static uk.gov.ons.ctp.response.notify.lib.MockMvcControllerAdviceHelper.mockAdviceFor;
...
public class EmailEndpointTest {
  ...
  private static final String VALID_JSON_BAD_EMAIL_ADDRESS =
      String.format(JSON_SKELETON, BAD_EMAIL_ADDRESS, MESSAGE_REFERENCE);
  private static final String VALID_JSON_VALID_EMAIL_ADDRESS =
      String.format(JSON_SKELETON, VALID_EMAIL_ADDRESS, MESSAGE_REFERENCE);
  ...
  public void setUp() throws Exception {
    ...
    this.mockMvc =
        MockMvcBuilders.standaloneSetup(emailEndpoint)
  ...
  public void emailInvalidJson() throws Exception {
    ResultActions actions =
        mockMvc.perform(postJson(String.format("/emails/%s", TEMPLATE_ID), INVALID_JSON));
  ...
  public void emailInvalidEmailAddress() throws Exception {
    ResultActions actions =
        mockMvc.perform(
            postJson(String.format("/emails/%s", TEMPLATE_ID), VALID_JSON_BAD_EMAIL_ADDRESS));
  ...
  public void emailHappyPath() throws Exception {
    ...
    ResultActions actions =
        mockMvc.perform(
            postJson(String.format("/emails/%s", TEMPLATE_ID), VALID_JSON_VALID_EMAIL_ADDRESS));
